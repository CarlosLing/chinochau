...
class TestUtilityEndpoints:
    ...
    @patch("backend.main.translate_google")
    def test_translate_api(self, mock_translate, client: TestClient, test_db):
    ...
    def test_translate_api_invalid_input(
        self, mock_translate, client: TestClient, test_db
    ):
    ...
    def test_pinyin_api(self, client: TestClient, test_db):
        """Test the pinyin API endpoint"""
        request_data = {"chinese": "你好"}

        response = client.post("/pinyin", json=request_data)
        assert response.status_code == 200
    ...
    def test_pinyin_api_multiple_characters(self, client: TestClient, test_db):
    ...
    def test_pinyin_api_invalid_input(self, client: TestClient, test_db):
